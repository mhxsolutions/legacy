<%@ Page Title="<%= this.WebSiteShortName %> | Administer Users" Language="C#" MasterPageFile="~/<%= this.WebSiteShortName %>.master" AutoEventWireup="true" CodeBehind="AdministerUsers.aspx.cs" Inherits="Web.Administration.AdministerUsers" %>
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <asp:SqlDataSource ID="UsersDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:DWS No Rep DataConnectionString %>"
        SelectCommand="SELECT [<%= this.SqlShortName %> User Details].[User Id], [DWS Rep Data].dbo.tbldestinationList.Destination + ': ' + [DWS Rep Data].dbo.tbldestinationList.City + ', ' + ISNULL([DWS Rep Data].dbo.tbldestinationList.State, '') AS Client FROM [<%= this.SqlShortName %> User Details] INNER JOIN [DWS Rep Data].dbo.tbldestinationList ON [<%= this.SqlShortName %> User Details].[Client Filter Destination Ref] = [DWS Rep Data].dbo.tbldestinationList.[Destination id] ORDER BY [<%= this.SqlShortName %> User Details].[User Id]"
        InsertCommand="INSERT INTO [<%= this.SqlShortName %> User Details] ([User Id], [Client Filter Destination Ref]) VALUES (@UserId, @Client)">
        <InsertParameters>
