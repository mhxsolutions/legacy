<%@ Page Language="C#" MasterPageFile="~/CustomerService.master" AutoEventWireup="true" Inherits="Customers__All_PlannedDestinationLoadHistory" 
    Title="BudwayDirect | Planned Load History" CodeBehind="PlannedDestinationLoadHistory.aspx.cs" CodeFile="PlannedDestinationLoadHistory.aspx.cs" %>
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" Runat="Server">
    <h2>
        Destination/Stop Load History 
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:DWS No Rep DataConnectionString %>"
        SelectCommand="SELECT [Load], [Plan Date], Origin, Action, Arrive, Stop, [Sch Arrive], [Final Destination], Shipper, [Shipper Ref], [PDF File], [Dest ref], [PDF File Name], [Stop ref], age FROM COPSLoadPlanHistory WHERE ([Stop ref] = @Shipper_Ref) AND (age <= @sel_age) OR ([Dest ref] = @Shipper_Ref) AND (age <= @sel_age) ORDER BY [Plan Date], [Load]">
        <SelectParameters>
