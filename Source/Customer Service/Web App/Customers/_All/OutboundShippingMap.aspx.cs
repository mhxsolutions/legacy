...
public partial class Customers__All_OutboundShippingMap : System.Web.UI.Page
{
     ...
     * 
		[Driver ID] INT,
		[Last Agilis Event Timestamp] DATETIME,
		[Last Agilis Event Status] VARCHAR(5),
		...
		[Last Agilis Position ID] INT,
		[Last Agilis Position Timestamp] DATETIME,
		[Driver Latitude] FLOAT,
		[Driver Longitude] FLOAT,
		[Shipper Latitude] FLOAT,
		[Shipper Longitude] FLOAT,
    ...
    private List<DeliveryInProcess> GetDeliveriesInProcess(int ClientFilter)
    {
        ...
        {
            using (SqlCommand Command = new SqlCommand("CopsOutboundShipping", Connection))
            {
                Connection.Open();
    ...
    private void ConfigureMap(double SiteLatitude, double SiteLongitude, ICollection<PushPin> Pins)
    {
        StringBuilder s = new StringBuilder(
            "<script type=\"text/javascript\">" + Environment.NewLine +
                ...
                "{" + Environment.NewLine +
                "    map = new Microsoft.Maps.Map(document.getElementById('map'), { credentials: 'AmYGBm_e-LmSXggU5836KmUHiAOH2D_0pmTd8eKMqiXiIQ9TQJjfP591MWLNDvoe', center: new Microsoft.Maps.Location(" + SiteLatitude + ", " + SiteLongitude + "), zoom: 12 });" + Environment.NewLine +
                "}" + Environment.NewLine +
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        foreach (DeliveryInProcess d in DeliveriesInProcess)
        {
            ...
            {
                if (d.DestinationLatitude != 0.0 && d.DestinationLongitude != 0.0)
                {
