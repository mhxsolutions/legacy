...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div class="contentDiv">
        ...
        <asp:SqlDataSource ID="_CustomReportsDatasource" runat="server" ConnectionString="<%$ ConnectionStrings:DWS No Rep DataConnectionString %>"
            SelectCommand="SELECT [MhxWeb_AssignableReports].[Report Id], [MhxWeb_AssignableReports].Title, [MhxWeb_AssignableReports].Description, [MhxWeb_AssignableReports].URL, [Cops Report Assignments].[Client Id], [DWS Rep Data].dbo.tbldestinationList.Destination FROM [MhxWeb_AssignableReports] INNER JOIN [Cops Report Assignments] ON [MhxWeb_AssignableReports].[Report Id] = [Cops Report Assignments].[Assignable Report Ref] INNER JOIN [DWS Rep Data].dbo.tbldestinationList ON [Cops Report Assignments].[Client Id] = [DWS Rep Data].dbo.tbldestinationList.[Destination id] WHERE (@ContextUserId = 166073081) OR ([Cops Report Assignments].[Client Id] = @ContextUserId) OR (@ContextUserId = 2145260460) ORDER BY [MhxWeb_AssignableReports].Title">
            <SelectParameters>
