<%@ Page Title="" Language="C#" MasterPageFile="~/UPA.master" AutoEventWireup="true" CodeBehind="ManageUsers.aspx.cs" Inherits="Web.Administration.ManageUsers" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <h2>Manage Users</h2>
    <dx:ASPxGridView ID="gvManageUsers" ClientInstanceName="gridManageUsers" runat="server" Width="98%" AutoGenerateColumns="False" DataSourceID="SqlDataSource1" KeyFieldName="UserId" SettingsPager-PageSize="20"
        ...
        <Columns>
            ...
            <dx:GridViewDataTextColumn FieldName="MobileAlias" Caption="Mobile" VisibleIndex="7" PropertiesTextEdit-MaxLength="16" Width="80"></dx:GridViewDataTextColumn>
            <dx:GridViewDataTextColumn FieldName="Comment" VisibleIndex="8"></dx:GridViewDataTextColumn>
            <dx:GridViewDataDateColumn FieldName="CreateDate" Caption="Created" VisibleIndex="9" Width="80" EditFormSettings-Visible="False"></dx:GridViewDataDateColumn>
            <dx:GridViewDataDateColumn FieldName="LastActivityDate" Caption="Last Activity" VisibleIndex="10" Width="100" EditFormSettings-Visible="False"></dx:GridViewDataDateColumn>
            ...
            <dx:GridViewCommandColumn ShowEditButton="True" ShowDeleteButton="false" VisibleIndex="12" ShowNewButton="true" Width="80">
                <CustomButtons>
        ...
        SelectCommand="
            ...
            (
	            SELECT U.UserId, U.UserName, U.MobileAlias,
		            ...
		            [aspnetdb].dbo.GetRoleName(UserName, 'UpaWeb') AS UserRole,
		            C.[Company Name] AS CompanyName, C.[Company ID] AS CompanyId,
		            CL.[Destination Id] AS ClientId,
		            [UPA No Rep Data].dbo.GetClientName(UserName, 'UpaWeb') as Client 
	            FROM dbo.aspnet_Users U  
		            JOIN dbo.aspnet_Applications A ON A.ApplicationId = U.ApplicationId  AND A.ApplicationName = 'UpaWeb' 
		            JOIN dbo.aspnet_Membership M ON M.UserId = U.UserId
		            LEFT JOIN [UPA No Rep Data].dbo.[UPA User Details] D ON D.[User Id] collate SQL_Latin1_General_CP437_BIN = U.UserName
		            LEFT JOIN [UPA Rep Data].[dbo].[tbldestinationList] CL ON CL.[Destination Id] = D.[Client Filter Destination Ref] 
		            LEFT JOIN [UPA No Rep Data].[dbo].[Company] C ON C.[Company ID] = CL.[Company ID Ref] 
            ) T
            ...
            IF NOT EXISTS(SELECT 1 FROM [UPA No Rep Data].dbo.[UPA User Details] U WHERE U.[User Id] = @UserName)
