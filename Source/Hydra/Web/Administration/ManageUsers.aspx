<%@ Page Title="" Language="C#" MasterPageFile="~/Hydra.master" AutoEventWireup="true" CodeBehind="ManageUsers.aspx.cs" Inherits="Web.Administration.ManageUsers" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <asp:ObjectDataSource ID="odsRoles" runat="server" TypeName="UserData" SelectMethod="GetRoles"></asp:ObjectDataSource>
    <asp:ObjectDataSource ID="odsClients" runat="server" TypeName="UserData" SelectMethod="GetClientList"></asp:ObjectDataSource>
...
(
	SELECT U.UserId, U.UserName, U.MobileAlias,
		...
		[aspnetdb].dbo.GetRoleName(UserName, 'Hydra') AS UserRole,
		C.[Company Name] AS CompanyName, C.[Company ID] AS CompanyId,
		CL.[Destination Id] AS ClientId,
		[DWS No Rep Data].dbo.GetClientName(UserName, 'Hydra') as Client 
	FROM dbo.aspnet_Users U  
		JOIN dbo.aspnet_Applications A ON A.ApplicationId = U.ApplicationId  AND A.ApplicationName = 'Hydra' 
		JOIN dbo.aspnet_Membership M ON M.UserId = U.UserId
		LEFT JOIN [DWS No Rep Data].dbo.[Hydra User Details] D ON D.[User Id] collate SQL_Latin1_General_CP437_BIN = U.UserName
...
IF NOT EXISTS(SELECT 1 FROM [DWS No Rep Data].dbo.[Hydra User Details] U WHERE U.[User Id] = @UserName)
