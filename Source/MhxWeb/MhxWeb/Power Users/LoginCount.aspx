<%@ Page Title="" Language="C#" MasterPageFile="~/Main.master" AutoEventWireup="true" CodeFile="LoginCount.aspx.cs" Inherits="Power_Users_LoginCount" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="Server">
    ...
    <div class="contentDiv">
        ...
        <dx:ASPxGridView ID="gvLoginHistory" runat="server" AutoGenerateColumns="false" DataSourceID="DataSource1" Width="100%">
            ...
            <Columns>
                ...
                <dx:GridViewDataDateColumn FieldName="LoginDate" VisibleIndex="2" UnboundType="DateTime"></dx:GridViewDataDateColumn>
                <dx:GridViewDataTextColumn FieldName="Times" VisibleIndex="3"></dx:GridViewDataTextColumn>
            </Columns>
        </dx:ASPxGridView>

        <asp:SqlDataSource ID="DataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:DWS No Rep DataConnectionString %>"
            SelectCommand="SELECT UserName, CAST(LoginDateTime AS DATE) as LoginDate, count(UserName) as Times FROM LoginHistory WHERE LoginDateTime BETWEEN @StartDate AND (@EndDate + 1) AND UserRole = @UserRole  AND ApplicationName = @AppName AND (UserName in (select [User Id] from dbo.[MhxWeb_UserDetails] where [Client Filter Destination Ref] = @ClientId)) GROUP BY UserName, CAST(LoginDateTime AS DATE) ORDER BY LoginDate DESC">
            <SelectParameters>
                ...
                <asp:Parameter Name="AppName" Type="String" DefaultValue="MhxWeb" />
            </SelectParameters>
        </asp:SqlDataSource>
