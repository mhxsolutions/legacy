...
<TITLE>RM railcars by product 30 day</TITLE>
</HEAD>
<BODY>
<%
...
%>
<%
If IsObject(Session("RM_railcars_by_product_30_day_rs")) Then
    Set rs = Session("RM_railcars_by_product_30_day_rs")
Else
    sql = "SELECT [ATSF].[Terminal], IIf([S]='L','Inbound','Outbound') AS IO, IIf([product type]>'',[product type],'unassigned') AS Product, Count([ATSF].[Car Number]) AS [CountOfCar Number]  FROM [ATSF Product Type] RIGHT JOIN ATSF ON [ATSF Product Type].[ATSF Product ID]=[ATSF].[Product Type Ref]  WHERE ((([ATSF].[daterel]) Between Date()-30 And Date()))  GROUP BY [ATSF].[Terminal], IIf([S]='L','Inbound','Outbound'), IIf([product type]>'',[product type],'unassigned')  ORDER BY [ATSF].[Terminal], IIf([S]='L','Inbound','Outbound'), IIf([product type]>'',[product type],'unassigned')   "
    Set rs = Server.CreateObject("ADODB.Recordset")
    rs.Open sql, conn, 3, 3
    If rs.eof Then
        rs.AddNew
    End If
    Set Session("RM_railcars_by_product_30_day_rs") = rs
End If
%>
<TABLE BORDER=1 BGCOLOR=#ffffff CELLSPACING=0><FONT FACE="Arial" COLOR=#000000><CAPTION><B>Railcars by product
    (30 days)</B></CAPTION></FONT>
