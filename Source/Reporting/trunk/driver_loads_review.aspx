...
<form id="form1" runat="server">
	...
	<asp:GridView id="GridView1" runat="server" AutoGenerateColumns="False" DataSourceID="SqlDataSource1" AllowSorting="True" CellPadding="4" ForeColor="#333333" GridLines="None">
		...
		<Columns>
			...
			<asp:boundfield DataField="Sch_Arrive" DataFormatString="{0:t}" HeaderText="Sch_Arrive" SortExpression="Sch_Arrive">
			</asp:boundfield>
	...
	<asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:DWS No Rep DataConnectionString %>" SelectCommand="SELECT [Fleet], [Driver], [Order], [Destination], [Load Ref] AS Load_Ref, [Plan Date] AS Plan_Date, [Sch Arrive] AS Sch_Arrive FROM [Planned loads by fleet and driver for today] WHERE (([Fleet] = @Fleet) AND ([control date] &gt;= @control_date) AND ([control date] &lt;= @control_date2)) ORDER BY [Driver], [control date], [Order]">
		<SelectParameters>
