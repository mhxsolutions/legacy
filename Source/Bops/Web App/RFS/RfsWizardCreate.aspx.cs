...
public partial class RFS_RfsWizardCreate : Page
{
    ...
    private const string _NextPage = "RfsWizardMain.aspx";
    ...
    protected void CreateRfsButton_Click(object sender, EventArgs e)
    {
        BopsRfs NewRfs = new BopsRfs();
        NewRfs.SalespersonRef = Convert.ToInt32(SalespersonList.SelectedValue);
        NewRfs.ShipperRef = Convert.ToInt32(ShipperList.SelectedValue);
        NewRfs.CommodityRef = Convert.ToInt32(CommodityList.SelectedValue);
        NewRfs.WorkTypeRef = Convert.ToInt32(WorkTypeList.SelectedValue);
        NewRfs.StatusRef = Convert.ToInt32(StatusList.SelectedValue);
        NewRfs.BeginDate = DateTime.Now;

        string ConfigurationFileName = Path.Combine(Request.PhysicalApplicationPath, _DwsNoRepData);
        SessionManager<string> Mgr = new SessionManager<string>();
        ISession NHibernateSession = Mgr.AddSession(_DwsNoRepData, ConfigurationFileName);
        NHibernateSession.Save(NewRfs);

        BopsRfsStakeholder NewShipperStakeholder = new BopsRfsStakeholder();
        NewShipperStakeholder.RfsRef = NewRfs.RfsId;
        NewShipperStakeholder.RoleRef = BopsRfsStakeholderRoleType.Shipper;
        NewShipperStakeholder.ClientRef = NewRfs.ShipperRef;
        NHibernateSession.Save(NewShipperStakeholder);

        Session[_ContextRfsId] = NewRfs.RfsId;
        Mgr.RemoveSession(_DwsNoRepData, true);

        Context.Response.Redirect(_NextPage);
    }
}
