...
public partial class RFS_RfsWizardRequestWarehouse : Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Mode.ToLower() == "edit")
            {
                ...
                if (ServiceDetails.PrivateBillingEventType == RfsBillingEventType.WarehouseIn)
                    PrivateWhenChargedList.SelectedValue = "Received";
                else
                    PrivateWhenChargedList.SelectedValue = "Shipped";
                if (ServiceDetails.PublicBillingEventType == RfsBillingEventType.WarehouseIn)
                    PublicWhenChargedList.SelectedValue = "Received";
                else
                    PublicWhenChargedList.SelectedValue = "Shipped";
    ...
    private void GetServiceDetails(BopsRfsServicesRequested Service)
    {
        ...
        switch (PublicWhenChargedList.SelectedValue)
        {
            case "Received":
                Service.PublicBillingEventType = RfsBillingEventType.WarehouseIn;
                break;
            case "Shipped":
                Service.PublicBillingEventType = RfsBillingEventType.WarehouseOut;
                break;
        }
        ...
        if (User.IsInRole("Administrators"))
        {
            ...
            switch (PrivateWhenChargedList.SelectedValue)
            {
                case "Received":
        ...
        else if ((Mode.ToLower() == "add") || (!Service.PrivateRateIsUnlinked && !User.IsInRole("Administrators")))
        {
            ...
            Service.PrivateBillingEventType = Service.PublicBillingEventType;
            Service.CalculationScope = RfsCalculationScopeType.Both;
        }
