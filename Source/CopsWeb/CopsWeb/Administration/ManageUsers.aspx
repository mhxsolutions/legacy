...
(
	SELECT U.UserId, U.UserName, U.MobileAlias,
		...
		[aspnetdb].dbo.GetRoleName(UserName, 'CopsWeb') AS UserRole,
		C.[Company Name] AS CompanyName, C.[Company ID] AS CompanyId,
		CL.[Destination Id] AS ClientId,
		[DWS No Rep Data].dbo.GetClientName(UserName, 'CopsWeb') as Client 
	FROM dbo.aspnet_Users U  
		JOIN dbo.aspnet_Applications A ON A.ApplicationId = U.ApplicationId  AND A.ApplicationName = 'CopsWeb' 
		JOIN dbo.aspnet_Membership M ON M.UserId = U.UserId
		LEFT JOIN [DWS No Rep Data].dbo.[Cops User Details] D ON D.[User Id] collate SQL_Latin1_General_CP437_BIN = U.UserName
...
IF NOT EXISTS(SELECT 1 FROM [DWS No Rep Data].dbo.[Cops User Details] U WHERE U.[User Id] = @UserName)
