...
public class UserCompany
{
    ...
    {
        string sqlText = "select top 1 D.Destination + ': ' + D.City + ', ' + ISNULL(D.State, '') AS Company from [DWS Rep Data].dbo.tbldestinationList D JOIN [DWS No Rep Data].dbo.[Cops User Details] U ON U.[Client Filter Destination Ref] = D.[Destination id] WHERE U.[User Id] = @UserName";
    ...
    {
        string sqlText = "SELECT TOP 1 U.[Client Filter Destination Ref] AS DestinationId FROM [DWS No Rep Data].dbo.[Cops User Details] U WHERE U.[User Id]= @UserName";
        string connectionString = ConfigurationManager.ConnectionStrings["DWS No Rep DataConnectionString"].ConnectionString;
    ...
    public static string GetCompanyIdByUserName(string userName)
    {
        string sqlText = @"
            ...
            FROM [DWS No Rep Data].dbo.[Cops User Details] D
    ...
    {
        string sqlText = "SELECT DISTINCT C.[Destination id] as ClientId, C.Destination + ': ' + C.City + ', ' + ISNULL(C.State, '') as Client FROM [DWS No Rep Data].dbo.[Cops User Details] U INNER JOIN [DWS Rep Data].dbo.tbldestinationList C ON U.[Client Filter Destination Ref] = C.[Destination id] ";
        string connectionString = ConfigurationManager.ConnectionStrings["DWS No Rep DataConnectionString"].ConnectionString;
    ...
    {
        string sqlText = "SELECT DISTINCT C.[Destination id] as ClientId, C.Destination + ': ' + C.City + ', ' + ISNULL(C.State, '') as Client FROM [DWS No Rep Data].dbo.[Cops User Details] U INNER JOIN [DWS Rep Data].dbo.tbldestinationList C ON U.[Client Filter Destination Ref] = C.[Destination id] WHERE C.[Company ID Ref] = " + companyId;
        string connectionString = ConfigurationManager.ConnectionStrings["DWS No Rep DataConnectionString"].ConnectionString;
        DataSet ds = new DataSet();
        try
        {
    ...
    {
        string sqlText = "DELETE [DWS No Rep Data].dbo.[Cops User Details] WHERE [User Id] = @UserName";
        string connectionString = ConfigurationManager.ConnectionStrings["DWS No Rep DataConnectionString"].ConnectionString;
    ...
    {
        string sqlText = @"IF NOT EXISTS(SELECT 1 FROM [DWS No Rep Data].dbo.[Cops User Details] U WHERE U.[User Id] = @UserName)
                            BEGIN INSERT INTO [DWS No Rep Data].dbo.[Cops User Details] VALUES (@UserName, @ClientId) END 
                            ELSE BEGIN UPDATE U SET U.[Client Filter Destination Ref] = @ClientId
                            FROM [DWS No Rep Data].dbo.[Cops User Details] U WHERE U.[User Id] = @UserName END";
