<%@ Page Title="SCM | Administer Users" Language="C#" MasterPageFile="~/SCM.master" AutoEventWireup="true" CodeBehind="AdministerUsers.aspx.cs" Inherits="Web.Administration.AdministerUsers" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <dx:ASPxGridView ID="grdUsers" runat="server" DataSourceID="UsersDataSource" KeyFieldName="UserId" ClientInstanceName="grdUsers"
        Theme="Office2010Blue" AutoGenerateColumns="False" OnRowInserting="grdUsers_RowInserting" OnCustomCallback="grdUsers_CustomCallback"
        ...
        <Columns>
            ...
            <dx:GridViewDataTextColumn FieldName="UserId" VisibleIndex="1" PropertiesTextEdit-ValidationSettings-RequiredField-IsRequired="true"></dx:GridViewDataTextColumn>
            ...
            <dx:GridViewDataComboBoxColumn FieldName="Client" VisibleIndex="2" PropertiesComboBox-DataSourceID="StakeholderList" PropertiesComboBox-TextField="Destination" PropertiesComboBox-ValueField="Destination Id" PropertiesComboBox-ValidationSettings-RequiredField-IsRequired="true">
                <SettingsHeaderFilter Mode="List"></SettingsHeaderFilter>
            </dx:GridViewDataComboBoxColumn>


            <%--<dx:GridViewDataTextColumn FieldName="Password" Caption="Password" VisibleIndex="3" Visible="false" EditFormSettings-Visible="True">
                <EditItemTemplate>
                    <dx:ASPxTextBox ID="txtPassword" runat="server" Password="true"></dx:ASPxTextBox>
                    <dx:ASPxHyperLink ID="hlPassword" runat="server" NavigateUrl="javascript:OnLinkClick();" Text="Change Password"></dx:ASPxHyperLink>
                </EditItemTemplate>
            </dx:GridViewDataTextColumn>--%>
            ...
            <%--<dx:GridViewDataTextColumn Caption="ChangePassword" UnboundType="String" VisibleIndex="4">
                <EditItemTemplate>
                    <a id="detailsButton" href="javascript:OnLinkClick();">View</a>
                </EditItemTemplate>
    ...
    <asp:SqlDataSource ID="UsersDataSource" runat="server" ConnectionString="<%$ ConnectionStrings:DWS No Rep DataConnectionString %>"
        SelectCommand="SELECT [SCM User Details].[User Id] as UserId, '' as Password, '' as Change, [Client Filter Destination Ref] as DestinationId, [DWS Rep Data].dbo.tbldestinationList.Destination + ': ' + [DWS Rep Data].dbo.tbldestinationList.City + ', ' + ISNULL([DWS Rep Data].dbo.tbldestinationList.State, '') AS Client FROM [SCM User Details] INNER JOIN [DWS Rep Data].dbo.tbldestinationList ON [SCM User Details].[Client Filter Destination Ref] = [DWS Rep Data].dbo.tbldestinationList.[Destination id] ORDER BY [SCM User Details].[User Id]"
        InsertCommand="INSERT INTO [SCM User Details] ([User Id], [Client Filter Destination Ref]) VALUES (@UserId, @Client)"
        UpdateCommand="UPDATE [SCM User Details] SET [Client Filter Destination Ref] = @Client WHERE [User Id] = @UserId"
        DeleteCommand="DELETE FROM [SCM User Details] WHERE [User Id]=@UserId">
        <InsertParameters>
    ...
    <asp:SqlDataSource ID="StakeholderList" runat="server" ConnectionString="<%$ ConnectionStrings:DWS Rep DataConnectionString %>"
        SelectCommand="SELECT [Destination id], Destination + ': ' + City + ', ' + ISNULL(State, '') AS Destination FROM tbldestinationList WHERE ([Parent Ref] = [Destination id]) AND ([Company ID ref] = 8)"></asp:SqlDataSource>
