...
namespace Scm.OpsCore.Legacy.NHibernate
{
    ...
    public class RfsDataContext : IRfsDataContext
    {
        ...
        {
            _dwsNoRepDataConnectionString = ConfigurationManager.ConnectionStrings["DwsNoRepData"].ConnectionString;
            _dwsRepDataConnectionString = ConfigurationManager.ConnectionStrings["DwsRepData"].ConnectionString;

            var dwsNoRepDataSessionFactory = new NHibernateSessionFactory(_dwsNoRepDataConnectionString);
            _dwsNoRepData = dwsNoRepDataSessionFactory.CreateSession();
            var dwsRepDataSessionFactory = new NHibernateSessionFactory(_dwsRepDataConnectionString);
        ...
        {
            var query = from s in _dwsNoRepData.Query<BopsRfsStatus>()
                select s;
            return query.ToList();
        }
        ...
        {
            var query = from s in _dwsNoRepData.Query<BopsRfsBillingStatus>()
                        select s;
            return query.ToList();
        }
        ...
        {
            var query = from s in _dwsNoRepData.Query<BopsRfsInvoiceStatus>()
                        select s;
            return query.ToList();
        }
        ...
        {
            var query = from s in _dwsNoRepData.Query<BopsRfsCalculationResultType>()
                        select s;
            return query.ToList();
        }
        ...
        {
            var query = from l in _dwsNoRepData.Query<BopsLoad>()
                        where l.LoadReference == loadReference
                        select l;
            return query.FirstOrDefault();
        }
        ...
        /// <inheritdoc/>
        public IList<BopsRfs> GetRfs(int companyId, int maxRows, int skipRows)
        {
            var query = from r in _dwsNoRepData.Query<BopsRfs>()
                where (companyId == 0 || companyId == r.CompanyRef)
                select r;

            return query.Take(maxRows != 0 ? maxRows : int.MaxValue).Skip(skipRows).ToList();
        }
        ...
        /// <inheritdoc/>
        public IList<BopsDestination> GetDestinations(int companyId, int maxRows, int skipRows)
        {
            var query = from d in _dwsRepData.Query<BopsDestination>()
                where (companyId == 0 || companyId == d.CompanyRef)
                select d;
        ...
        /// <inheritdoc/>
        public IList<BopsRfsServicesOffered> GetServicesOffered(int companyId)
        {
            var query = from s in _dwsNoRepData.Query<BopsRfsServicesOffered>()
                where (companyId == 0 || companyId == s.CompanyRef)
                select s;
        ...
        /// <inheritdoc/>
        public IList<BopsRfsServicesRequested> GetServicesRequested(int companyId, int maxRows, int skipRows)
        {
            var query = from r in _dwsNoRepData.Query<BopsRfsServicesRequested>()
                join o in _dwsNoRepData.Query<BopsRfsServicesOffered>() on r.ServiceOfferedRef equals o.Id
                where (companyId == 0 || companyId == o.CompanyRef)
                select r;
        ...
        public IList<BopsRfsInvoice> GetInvoicesByDates(DateTime beginDate, DateTime endDate, int companyId)
        {
            var query = from i in _dwsNoRepData.Query<BopsRfsInvoice>()
                where beginDate <= i.InvoiceDate &&
                      i.InvoiceDate < endDate &&
                      (companyId == 0 || i.CompanyRef == companyId)
                select i;
        ...
        /// <inheritdoc/>
        public IList<BopsRfsStakeholder> GetStakeholders(int companyId, int maxRows, int skipRows)
        {
            var query = from s in _dwsNoRepData.Query<BopsRfsStakeholder>()
                where (companyId == 0 || companyId == s.CompanyRef)
                select s;
        ...
        public IList<BopsRfsServiceCalculationResult> GetServiceCalculationResultsByDatesAndBillingStatus(DateTime beginDate, DateTime endDate,
            RfsBillingStatus billingStatus, int companyId)
        {
            var query = from r in _dwsNoRepData.Query<BopsRfsServiceCalculationResult>()
                        where beginDate <= r.BillingDate &&
                              r.BillingDate < endDate &&
                              r.BillingStatus == billingStatus &&
                              (companyId == 0 || r.CompanyRef == companyId)
                        select r;
        ...
        /// <inheritdoc/>
        public IList<BopsRfsLocation> GetLocations(int companyId)
        {
            var query = from l in _dwsNoRepData.Query<BopsRfsLocation>()
                where (companyId == 0 || companyId == l.CompanyRef)
                select l;
        ...
        public void AddInvoiceExportDetail(BopsRfsInvoiceExportDetail newInvoiceExportDetail)
        {
            _dwsNoRepData.Save(newInvoiceExportDetail);
        }
        ...
        /// <inheritdoc/>
        public void AddCalculationResult(BopsRfsCalculationResult newCalculationResult)
        {
            _dwsNoRepData.Save(newCalculationResult);
        }
        ...
        /// <inheritdoc/>
        public IList<BopsScan> GetScansByDocumentIdsSignedAndReject(string[] loadRefs, bool isSigned, int rejectRef)
        {
            /*
        ...
        public BopsRfsServicesOffered GetServiceOfferedByServiceRequestedId(int serviceRequestedId)
        {
            var query = from r in _dwsNoRepData.Query<BopsRfsServicesRequested>()
                        join o in _dwsNoRepData.Query<BopsRfsServicesOffered>() on r.ServiceOfferedRef equals o.Id
                        where (r.ServiceRequestedId == serviceRequestedId)
                        select o;
        ...
        {
            var query = from f in _dwsNoRepData.Query<BopsFsc>()
                        select f;
            return query.ToList();
        }

        public BopsFsc GetFuelSurchargeById(int fscId)
        {
            var query = from f in _dwsNoRepData.Query<BopsFsc>()
                        where f.FscId == fscId
                        select f;
        ...
        public IList<BopsCompany> GetCompanies()
        {
            return _dwsNoRepData.CreateCriteria(typeof(BopsCompany)).List<BopsCompany>();
        }

        public IList<BopsCompanyUser> GetCompanyUsers()
        {
            return _dwsNoRepData.CreateCriteria(typeof(BopsCompanyUser)).List<BopsCompanyUser>();
        }

        public IList<BopsCompanyUserPermission> GetCompanyUserPermissions()
        {
            return _dwsNoRepData.CreateCriteria(typeof(BopsCompanyUserPermission)).List<BopsCompanyUserPermission>();
        }

        public void UpdateStorageBillingInventoryTracking(int inventoryId, DateTime newDateBilledThrough)
        {
            var tracking = _dwsNoRepData.Load<BopsRfsStorageBillingInventoryTracking>(inventoryId);
            if (tracking == null)
            {
                var newTracking = new BopsRfsStorageBillingInventoryTracking()
                {
                    InventoryRef = inventoryId,
                    DateBilledThrough = newDateBilledThrough
                };

                _dwsNoRepData.Save(newTracking);
            }
            else
            {
                tracking.DateBilledThrough = newDateBilledThrough;
            }
        }
